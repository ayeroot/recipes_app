...
suppression persistants, et thème **clair / sombre / système**. Conçue
pour valider la maîtrise des widgets Flutter et de la navigation.
...
- **Favoris persistants** (survivent au redémarrage), avec retrait par
  glissement et « Annuler » via SnackBar.
...
- **Interface responsive à 3 niveaux** :
  ...
  - grand écran / desktop (≥ 840 px) → grille 3 colonnes **et** barre de
    navigation latérale (`NavigationRail`).
- **Route d'erreur (404)** si un lien ne correspond à aucun écran.
- **Tests automatisés** (recherche, filtrage, navigation).

## 🧱 Widgets utilisés (≥ 8 types différents)

`ListView`, `GridView`, `Stack`, `Card`, `ChoiceChip` / `Chip`,
`TextFormField`, `DropdownButtonFormField`, `NavigationBar`,
`NavigationRail`, `CustomScrollView` / `SliverAppBar`, `RadioListTile`,
`FloatingActionButton`, `IndexedStack`, `Dismissible`,
`PopupMenuButton`, `AlertDialog`, `SnackBar` (avec `SnackBarAction`),
entre autres.
...
└── widgets/
    ...
    └── empty_state.dart
test/
└── widget_test.dart              # Tests : recherche, filtrage, navigation
```

> **Séparation UI / données stricte** : les widgets et écrans ne
...
- Un appareil, un émulateur, ou un navigateur (Flutter Web)
...
- **Flutter** (Material 3)
- **go_router** — navigation déclarative (routes nommées, routes
  paramétrées, gestion des routes inconnues)
